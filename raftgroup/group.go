...
type RaftApplyHandler func( /*req*/ *raft_cmdpb.RaftCmdRequest, uint64) ( /*resp*/ *raft_cmdpb.RaftCmdResponse /*err*/, error)
...
func (rg *RaftGroup) SubmitCommand(ctx context.Context, req *raft_cmdpb.RaftCmdRequest) (*raft_cmdpb.RaftCmdResponse, error) {
	cmd, err := req.Marshal()
	if err != nil {
		return nil, err
	}
	...
	if rsp, ok := resp.(*raft_cmdpb.RaftCmdResponse); ok {
		return rsp, nil
	}
	return nil, errUnknownResponseType
}

func (rg *RaftGroup) ChangePeer(ctx context.Context, typ schpb.RangePeerOp, nodeId uint64) error {
	ccPeer := raftproto.Peer{Type: raftproto.PeerNormal, ID: nodeId}
	var ccType raftproto.ConfChangeType

	switch typ {
	case schpb.RangePeerOp_Op_Add:
		ccType = raftproto.ConfAddNode
	case schpb.RangePeerOp_Op_Remove:
		ccType = raftproto.ConfRemoveNode
	default:
		return errors.New("unknown member change type")
	}
	future := rg.raftServer.ChangeMember(ctx, rg.id, ccType, ccPeer, nil)
	resp, err := future.Response()
	if err != nil {
		return err
	}
