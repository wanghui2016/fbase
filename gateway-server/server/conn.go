...
func (c *ClientConn) Handshake() error {
	if err := c.writeInitialHandshake(); err != nil {
		log.Error("server", "Handshake", err.Error(),
			c.connectionId, "msg", "send initial handshake error")
		return err
	}

	if err := c.readHandshakeResponse(); err != nil {
		log.Error("server", "readHandshakeResponse",
...
func (c *ClientConn) Run() {
	...
	for {
		...
		if err != nil {
			log.Error("read packet err:%s", err.Error())
			return
		}
		if log.GetFileLogger().IsEnableDebug() {
			log.Debug("read packet success:%s", hack.String(data))
		}

		if err := c.dispatch(data); err != nil {
			log.Error("server run error(%v), connetionId=%v", err.Error(), c.connectionId)
			c.writeError(err)
