...
// 创建一个只处理一个表的Proxy
func newTestProxy(columns []*columnInfo, ranges []*util.Range) *Proxy {
	...
	dc := gmock.NewDataServerCli("", ranges)
	...
	p := &Proxy{
		msCli:   mc,
		nodeCli: dc,
		router:  NewRouter(mc, []string{fmt.Sprintf("127.0.0.1:%d", port)}),
