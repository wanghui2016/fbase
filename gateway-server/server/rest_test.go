...
func TestRestKVHttp(t *testing.T) {
	...
	conf := &Config{
		...
		SignKey:           "sharkstore-masterserver",
		MaxClients:        10000,
		User:              "fbase",
		Password:          "123123",
		Charset:           "utf8",
...
func TestCommandTranslateValue(t *testing.T) {
	id := func (n int) []byte {
		tmp := int64(n)
            ...
            "and": [
                {
                    "field": {
                        "column": "id",
                        "value": "`+base64.StdEncoding.EncodeToString(id)+`"
                    },
                    "relate": ">"
                }
            ]
	...
	if err := json.Unmarshal([]byte(queryJson), q); err != nil {
		t.Fatal("query unmarshal error: ", err)
	}
	fmt.Printf("query: %v\n", *q.Command.Filter.And[0].Field)

	colMap := make(map[string]*metapb.Column)
	colMap["id"] = &metapb.Column{
		Name: "id",
		DataType: metapb.DataType_Int,
	}
	if err := q.commandTranslateValue(colMap); err != nil {
		t.Fatal("translate error: %v", err)
	}
	fmt.Printf("query: %x\n", q.Command.Filter.And[0].Field.Value.(int64))

	if q.Command.Filter.And[0].Field.Value.(int64) != 0x1234 {
		t.Fatal("!= 0x1234")
	}
