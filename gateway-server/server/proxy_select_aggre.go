...
func (p *Proxy) selectAggre(t *Table, kvproxy *KvProxy, req *kvrpcpb.KvSelectRequest) ([][]*kvrpcpb.Row, error) {
	...
	if req.Scope.Start == nil {
		...
		start = &metapb.Key{Key: req.Scope.Start, Type: metapb.KeyType_KT_Ordinary}
	}
	if req.Scope.Limit == nil {
		limit = &metapb.Key{Key: nil, Type: metapb.KeyType_KT_PositiveInfinity}
	} else {
		limit = &metapb.Key{Key: req.Scope.Limit, Type: metapb.KeyType_KT_Ordinary}
	}
