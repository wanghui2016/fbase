...
func NewServer(cfg *Config) (*Server, error) {
	...
	s.listener = l
	proxy := NewProxy(cfg.MasterServerAddrs, cfg.MasterManageAddrs, cfg)
	if proxy == nil {
	...
	svr.Init("gateway", config, nil)
	svr.Handle("/debug/log/setlevel", server.ServiceHttpHandler{Handler: func(w http.ResponseWriter, r *http.Request) {
		log.SetLevel(r.FormValue("level"))
		w.Write([]byte("OK"))
	}})
	svr.Handle("/kvcommand", server.ServiceHttpHandler{Handler: s.handleKVCommand})
	svr.Handle("/tableinfo", server.ServiceHttpHandler{Handler: s.handleTableInfo})
	go svr.Run()
	s.httpSvr = svr
...
func (s *Server) onConn(c net.Conn) {
	...
	defer func() {
		...
		if err != nil {
			...
			log.Error("server", "onConn", "error", 0,
	...
	if err := conn.Handshake(); err != nil {
		log.Error("server", "onConn", err.Error(), 0)
		conn.writeError(err)
		conn.Close()
		return
	}
...
func (s *Server) heartbeat() {
	id := func() string {
		for _, master := range s.cfg.MasterServerAddrs {
			conn, err := net.Dial("udp", master)
			if err != nil {
				log.Error("can not connect to master %v", master)
				continue
			}
			defer conn.Close()
	...
	if len(id) == 0 {
		log.Panic("none of master can not be connected")
	}

	for {
		select {
		case <-time.After(time.Duration(s.cfg.HeartbeatIntervalSec) * time.Second):
			stats_ := s.proxy.stats
			s.proxy.stats = NewStats(s.cfg)
			//log.Debug("gateway hb....")
			if _, err := s.proxy.msCli.GatewayHeartbeat(&mspb.GatewayHeartbeatRequest{
				Addr: id,
				Stats: &statspb.SqlStats{
					Ops:      stats_.tp.Calls / ((stats_.callUsec / 1000000) + 1),
					DelayAvg: stats_.callUsec / (stats_.tp.Calls + 1),
					Tp: func() (result []byte) {
						data, err := proto.Marshal(stats_.tp)
						if err != nil {
							log.Error("tp mashal error: %v", err)
							return
						}
						var buf bytes.Buffer
						w, err := flate.NewWriter(&buf, -1)
						if err != nil {
							log.Error("tp mashal new writer error: %v", err)
							return
						}
						w.Write(data)
						w.Close()
						result = buf.Bytes()
						//log.Debug("calls: %v, usec: %v, tp: %v", stats_.tp.Calls, stats_.callUsec, *stats_.tp)
						//log.Debug("ops: %v", stats_.tp.Calls/((stats_.callUsec/1000000)+1))
						//log.Debug("delay average: %v", stats_.callUsec/(stats_.tp.Calls+1))
						//log.Debug("tp str: %v", result)
						return
					}(),
				},
				Time: time.Now().Unix(),
				...
				// TODO
				log.Error("heartbeat error: %v", err)
			}
		}
	}
}
