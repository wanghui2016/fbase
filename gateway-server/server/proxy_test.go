...
func TestProxyInsertRows3(t *testing.T) {
	...
	testProxySelect(t, p, expected, "select * from "+testTableName)
	for i := 0; i < 30; i++ {
		sqql := fmt.Sprintf("insert into %s (id,name) values ('aa%d', 'myname')", testTableName, i)
		testProxyInsert(t, p, 1, sqql)
	}
	expected = [][]string{[]string{"39"}}
	testProxySelect(t, p, expected, "select count(*) from "+testTableName+" limit 1")
	expected = [][]string{[]string{"31"}}
	testProxySelect(t, p, expected, "select count(*) from "+testTableName+" where id < 'b'")
	expected = [][]string{[]string{"32"}}
	testProxySelect(t, p, expected, "select count(*) from "+testTableName+" where id < 'c'")
	expected = [][]string{[]string{"8"}}
	testProxySelect(t, p, expected, "select count(*) from "+testTableName+" where id >= 'b'")
	expected = [][]string{[]string{"6"}}
	testProxySelect(t, p, expected, "select count(*) from "+testTableName+" where id >= 'b' and id < 'h' ")
}
