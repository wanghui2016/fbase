...
func NewRouter(cli client.Client, msAddrs []string) *Router {
	router := &Router{
		cli:     cli,
		msAddrs: msAddrs,
...
func (rr *Router) findDbFromRemote(dbname string) *DataBase {
	...
	return NewDataBase(_db, rr.cli, rr.msAddrs)
}
...
func (rr *Router) FindRoute(dbName, tableName string, key []byte) (*metapb.Route, error) {
	db := rr.FindDB(dbName)
	if db == nil {
...
//func (rr *Router) FindRouteByRangeId(dbName, tableName string, rangeId uint64) (*Route, error) {
//	db := rr.FindDB(dbName)
//	if db == nil {
//		return nil, errors.New("invalid database")
//	}
...
//		return nil, errors.New("invalid table")
//	}
//	route := table.FindRouteById(rangeId)
//	return route, nil
//}
