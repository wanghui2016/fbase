...
func TestLogStorage1(t *testing.T) {
	...
	ents, isCompact, err = s.Entries(lo, hi, 0)
	if err != nil {
		t.Error(err)
	}
	if isCompact {
	...
	err = compareEntries(ents, toWrite[:1])
	if err != nil {
		t.Error(err)
	}

	ents, isCompact, err = s.Entries(lo, hi, 1024*1024)
	if err != nil {
		t.Error(err)
	}
	if isCompact {
	...
	ents, isCompact, err = s.Entries(20, 50, 10*toWrite[0].Size())
	if err != nil {
		t.Error(err)
	}
	if isCompact {
	...
	ents, isCompact, err = s.Entries(lo, hi+1, math.MaxUint64)
	if err != nil {
		t.Error(err)
	}
	if isCompact {
	...
	ents, isCompact, err = s.Entries(20, 50, toWrite[0].Size()*10)
	if err != nil {
		t.Error(err)
	}
	if isCompact {
...
func TestLogOpenTruncate(t *testing.T) {
	...
	// truncate后再写
	ents2, isCompact, err := s.Entries(lo, i, math.MaxUint64)
	if err != nil {
		t.Fatal(err)
	}
	if isCompact {
...
func TestTruncateOld(t *testing.T) {
	...
	ents, isCompact, err = s.Entries(101, hi, math.MaxUint64)
	if err != nil {
		t.Fatal(err)
	}
	if isCompact {
...
func TestTruncateCorrupt(t *testing.T) {
	var err error
	dir, err := ioutil.TempDir(os.TempDir(), "fbase_test_rlog_truncate_corrupt_")
	if err != nil {
		t.Fatal(err)
	}
	t.Logf("TestPath: %v", dir)
	...
	s.ls.last.f.Truncate(info.Size() - int64(1))
	if err := s.ls.Sync(); err != nil {
		t.Fatal(err)
	}
	s.Close()
	...
	ents2, isCompact, err := s.Entries(lo, hi, math.MaxUint64)
	if err != nil {
		t.Fatal(err)
	}
	if isCompact {
		t.Fatal("expect not compact")
	}
	// 应该少一条
	if len(ents2) != 19 {
		t.Fatalf("expect entry len:%d, actual:%d", 19, len(ents2))
	}
	err = compareEntries(ents[:len(ents)-1], ents2)
	if err != nil {
		t.Error(err)
	}

	if err := s.StoreEntries(ents[len(ents)-2:]); err != nil {
		t.Fatal(err)
	}
	ents2, isCompact, err = s.Entries(lo, hi, math.MaxUint64)
	if err != nil {
...
// 	// hsf, hs, meta, err := openMetaFile("/home/jonah/Downloads/export/Data/data-server/range_34/raft")
// 	hsf, hs, meta, err := openMetaFile("/home/jonah/Downloads/export/Data/data-server/range_4162/raft")
// 	if err != nil {
// 		t.Error(err)
// 	}

// 	_ = hsf
// 	t.Logf("%d %d %d -- %d %d", hs.Commit, hs.Term, hs.Vote, meta.truncIndex, meta.truncTerm)

// 	// hsf.SaveHardState(hs)
// 	// hsf.SaveTruncateMeta(truncateMeta{580514733, 257})
// 	// hsf.Sync()
// 	// hsf.Close()
